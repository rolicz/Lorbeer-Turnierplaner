...
- **The keyboard owns the bottom of the screen.** Everything pinned there clears the mobile
  ...
  `ui/shell/keyboardOpen.ts`, Q2). A composer's sticky offset, the filter pill and the error toast
  therefore collapse in the same repaint, so nothing floats in a 72px gap over the keys. The
  page's own end padding is the other token, `nav-h`, and it deliberately does **not** collapse:
  shortening the document mid-sentence drags the caret down with it. Two surfaces hide with the
  bar (the bar itself, the filter pill) and one never does — the error toast, because an error you
  cannot see is worse than a filter you cannot reach.
