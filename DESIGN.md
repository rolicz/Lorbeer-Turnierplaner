...
>
> Last checked against the code: **2026-09-23** (L15, the auth batch's one documentation pass, on
...
**Set a password** (Settings, with and without one), **Remove password**. A logged-in browser is a
...
password hint is `At least 10 characters`, nothing else. A name that may not be opened is titled
...
is taken*, *That passkey could not be used to log in*. A 429 is `Too many attempts — try again in
...
| A screen before the app | `AuthScreen` (`pages/auth/`) | the bare layout every auth page shares — `/login`, `/register`, `/reset`, and "not in a group yet" — rendered **outside** `AppShell`: no top bar, no tab bar, no sidebar, no toast viewport. `min-h-screen bg-bg-default` with the body's safe-area padding, one `max-w-sm mx-auto px-4` column, the 96px `rounded-2xl` logo (`/icon-512.png`) and "Lorbeerkranz" in `text-xl font-semibold` above **one** `card`, and a `below` slot under the card for the muted "see more" lines (§6: "New here? Register with a code ›", "Already have an account? Log in ›", "Know your password? Log in ›"). Measured at 390 / 1280 in both themes: card 358 / 384px wide, centred to the pixel, `a a` = 0 |
...
| Setting a password from a link | `ResetPage` (`pages/auth/`) | reads the token from the fragment (or `?token=`) **once** and strips it from the address bar before the first paint and before the only request (the router's `replaceState`); a reload therefore shows the "This link is not complete — ask the admin for a new one." `EmptyState`, by design — the link works once and is kept nowhere. One `PasswordField` and a full-width solid **Set password** |
| A passkey on the login screen | a ghost `Button` "Use a passkey" (`Fingerprint`) | full width, **after** the password form and a `divider` — a person with a password is not made to hunt for it — and rendered only where `browserSupportsWebAuthn()`, which hides it on the phone against dev (plain http is not a secure context). **A cancelled system sheet is not an error**: a closed Face ID / passkey sheet (`NotAllowedError`) says nothing at all, anywhere in the app; a refusal is the server's one sentence on the form's error line |
| My account | `SecuritySection` (`pages/settings/`) in `SettingsSection` cards, Settings → Account | four groups, in this order. **Devices**: a `ListRow` per session, `Since <date> · last seen <date, time>`, the current one wearing a `pill-default` **this device** and dropping "last seen" (it is in use now; at 390px the pill truncated the line otherwise) and carrying **no** sign-out; every other row a ghost `LogOut` icon button → `ConfirmDialog` **without** a red block (reversible: log in again); a footer ghost "Sign out other devices" only while another device exists. **Passkeys**: a row per passkey (label, `Added <date> · last used <date, time>` or `· not used yet`, a `pill-default` "synced" when backed up) with a ghost `Trash2` → `ConfirmDialog` **with** the red block, subtitle "Every device will be signed out, including this one." — after it the app logs out and lands on `/login` without the "session has ended" line (nobody was surprised); **the last way in** (no password, one passkey) is a disabled button whose title *and* a line under the list carry the server's own 409 sentence, because a tooltip does not exist on a phone; then an optional **Name** field and a full-width solid **Add a passkey** (`KeyRound`), replaced by "This browser cannot make passkeys." where WebAuthn is missing. **Password**: read view or editor, never both (§9b) — a full-width ghost toggle **Change password** / **Set a password** with a chevron and `aria-expanded`; the editor is the current field (only when there is a password), the new `PasswordField`, one full-width solid **Save**, the error line and `RetryCountdown`; the toggle is the way out and discards the draft. "This is the password you were given — change it, or add a passkey." in `text-warn` above it while the password is the migrated one and no passkey exists. **Remove password** is a muted full-width ghost button, only with a password *and* a passkey, `ConfirmDialog` **without** a red block (reversible by "Set a password" right there). **Groups**: a `ListRow` per membership with the role as a `pill-default` `Pill`, then `InviteCodeField` + a ghost **Join**. Measured at 390 / 1280 in both themes: the cards at x=16 / w=358 and x=424 / w=672 |
| "Secure your account" | `SecureAccountNotice` (`ui/shell/`) | the `PushSetupNotice` shape — first in `<main>`, above the route, `card mb-3 border-warn/40 bg-warn/10` (a `.card`, never an `.inset`, so light keeps the tone, §3), a `KeyRound` in `text-warn`, "Secure your account — add a passkey." and one solid small `Link` **Add a passkey** to `/settings?tab=account` carrying `NAV_JUMP_STATE`. `warn` because nothing failed (§2). **Not dismissible** (Roli): it stops by itself. **Its condition — the one place it is written, and awaiting Roli's answer (`AGENTS.md` §11):** shown iff the session is `authed` **and** `passwordMigrated` (the password is still the one `secrets.json` gave) **and** `!hasPasskey` **and** `passkeysSupported()`, and never on Settings → Account itself or on the auth screens. So changing the migrated password ends it as well as adding a passkey; the alternative Roli has been asked about is "no passkey" alone, which would also show it to accounts that registered with a password — one line in the component and one here. It lives in the page column, so it cannot move the top bar's title (Q13, measured 195.0 with and without). Measured: 358×66 at 390 (the tab strip moves +78px, i.e. 66 + the column's 12px margin) and 992×58 at 1280 (+70px), in both themes |
| The admin page | `AdminPage` (`pages/admin/`), `/admin`, owner+ | `PageLayout title="Admin"` with `SectionTabs` **Accounts** (`Users`, default) · **Invites** (`Ticket`), the body in `mx-auto w-full max-w-2xl`. *Accounts*: `section-head` "Accounts", a `ChipGroup` **All** / **Logged in** / **Migrated password**, then one `List` of `ListRow`s — `AvatarCircle h-10 w-10` (**no cups**: this is not a present-tense surface), the name with a `pill-default` **site admin** or **owner**, and a subtitle that **leads with the login state** (`migrated password` in `text-warn`, or `no login`) before `N devices · last seen <date, time>`, because at 390px the tail is what truncates and the state is what Roli asked to see. Trailing `size="sm" iconOnly` ghosts: `UserCog` (make/remove owner, owner+, only on rows whose role is `owner` or `editor`) then `KeyRound` (a reset link, **site admin only**). The row's own tap opens `SessionsSheet` for the site admin only; for an owner the row is not a control at all (the server would 403 the sheet — no overlay, no chevron). *Invites*: `section-head` "Invite codes", a note `Input`, a **full-width** solid **Create code**, the `ShownOnce` box, then "Live codes" as a `List` (note or "No note", `created <ts> · by <name> · expires in N min`, a `Trash2` revoke — **with** the red block, because it deletes a stored row). The title "Admin" centres at 160.0 / 195.0 / 215.0 on the phone (Q13) |
...
- **A password is edited in place, like everything else** (L7). Settings → Account → Password is a
  ...
  The worked example is §7's "My account" row.
...
- A **destination** — anything the bottom bar, the sidebar or the drawer points at (Dashboard,
  ...
  (`/login`, `/register`, `/reset`, which render outside the shell and have no back affordance)
