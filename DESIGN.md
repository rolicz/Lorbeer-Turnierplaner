...
>
> Last checked against the code: **2026-09-24** (E6, the email half's documentation pass, on
> `feature/2026-09-auth` at `c4a5f21` — not deployed yet, `FEATURES_2026-09-auth-email.md`
> E0–E5. Changed here: §5b gains the email half's words (**Create a passkey**, **Use a password
> instead**, **Set a new login**, **Get back in**, **Send me a link**, **Confirm**, **Email**,
> **verified**, **pending**, **Send verification link**, **Send again**, **Change** / *Change email*,
> **Remove email**, **Not secured**), the recovery sentence, the four email refusals, and a password
> hint of `At least 15 characters`; §7 gains `PasskeyOrPassword` (the one credential choice — the
> passkey first), `RecoverPage`, `VerifyEmailPage` (a button, not an automatic POST), the login's
> recovery link and cross-device hint, and rewrites the reset row, "My account" (the **Email** group
> and the passkey hint), the admin row (**Not secured**, the email words, the mail-status line) and
> **`SecureAccountNotice` — its condition is decided (verified email *and* a secure login, the email
> step only while the server can send) and written there with its five sentences, closing the open
> question the pass below left**; §9b learns that the Email group edits in place like the password;
> §10's auth screens gain `/recover` and `/verify-email`. The numbers are E3's and E4's
> measurements, not re-measured here.)
> The pass before it was **2026-09-23** (L15, the auth batch's one documentation pass, on
> `feature/2026-09-auth` — not deployed yet. Changed here: §5b gains the auth words (`Log in`,
> `Register`, `Join`, `Set password`, `Invite code`, `Devices`, `this device`, `passkey`, `synced`,
> …); §7 gains the auth screens (`AuthScreen`, the form's own error line, `RetryCountdown`,
> `PasswordField`, `InviteCodeField`, the reset page), Settings → Account, the admin page and
> `ShownOnce`, `SecureAccountNotice` — **whose condition is written in that row and nowhere else,
> awaiting Roli's answer** — and "a cancelled system sheet is not an error"; §7's `PlayerLink` and
> `ListRow` rows learn who may open a profile, the `stretched` variant and the `overlay` slot; §10
> learns `/admin` and the basename; §11 records that the last `role="button"` is gone. Every number
> in those rows is a worker's measurement from `FEATURES_2026-09-auth.md`, not re-measured here.)
> The pass before it was **2026-09-23** (Q-E, the bell that admits a denied permission, on
> `fix/2026-09-bell-denied`. Changed here: §7's `NotificationBell` row gains the blocked mark and
> its one strip of copy, §10.5 gains the rule that the bell says things about itself *inside* its
> own box, and §5b gains the word `blocked` and the sentence that names where to fix it. The title
> centre was re-measured on all nine top-bar routes at 320 / 390 / 430px, with and without the
> mark. Nothing else was re-read this pass.)
> The pass before it was **2026-09-20** (Q-D, the composer behind the keyboard, on
...
**A box that carries a *tone* is not an `.inset`** (Q-E, measured). `[data-theme="light"] .inset`
repaints the background at specificity (0,2,0), which beats a `bg-warn/10` or `bg-error/10`
utility (0,1,0) — so `inset bg-warn/10` shows its tone in `blue` and loses it in `light`, which is
exactly the theme where a warning most needs to be visible. Write the box out instead:
`rounded-xl border border-warn/40 bg-warn/10 p-3` (the shape the push settings' error box has
always used). Same geometry, same layer, and it reads the same in every theme.
...
(inside a sentence) — so the buttons are **Log in** and **Log out** (the "Login" button went with
the reader, L4).
**The auth words** (L4–L9). The login field is **Name** — the login identifier *is* the display
name — and registration asks for a **Display name** ("The name the others will see"). **Invite
code** (shown `ABCD-EFGH`), **Register**, **Register with a code** (the login screen's way there),
**Join** (redeem a code into a group), **Set password** (the reset page), **Change password** /
**Set a password** (Settings, with and without one), **Remove password**. **Where a credential is
chosen** — register, a reset link, Settings — the passkey comes first (E3): **Create a passkey**
(solid), then **Use a password instead** (ghost), and **Create a passkey instead ›** as the way
back; a page opened from a reset or recovery link is **Set a new login**. Recovery is **Get back
in** (heading), **Send me a link** (button) and **Lost your passkey or password? ›** (the login
screen's way there); its one answer, before and after, for every address, is *If that address is
verified, a link is on its way. Check your spam folder too.* A verify link's page is **Confirm your
email** with one **Confirm**, then *Verified. You can close this and go back to the app.* The
settings group is **Email**, its states **verified** and **pending** (`pill-default`), its buttons
**Send verification link**, **Send again**, **Change** — whose accessible name is **Change email**,
because **Change password** sits one card above — and **Remove email**; the admin chip is **Not
secured**, the admin subtitle's words `no email` / `email pending`. A logged-in browser is a
**device**: **Devices**, **this device** (a `Pill` on the current one), **Sign out <device>**,
**Sign out other devices**, **Sign out every device** (the admin sheet). A **passkey** is lower
case in a sentence: **Add a passkey**, **Use a passkey**, **Remove passkey**, **Remove this
passkey** (the row's button title), and **synced** for one iCloud Keychain reports backed up. A
password hint is `At least 15 characters`, nothing else (a **new** password; login never checks
length, so nothing in the app ever shows a floor to someone typing an existing one). A name that may not be opened is titled
**Not in your group**; a profile page refused by the server says **This profile is in another
group.** Refusals the server writes are shown verbatim and are the only words for their case:
*Wrong username or password*, *That code is not valid*, *That reset link is not valid*, *That name
is taken*, *That passkey could not be used to log in*, and since the email half *That passkey could
not be registered*, *That passkey is already registered*, *That link is not valid*, *That does not
look like an email address*, *That email address is used by another account* and *Could not send
the email — try again in a moment*. What the *browser* refused while making a passkey (an
authenticator that already holds this account's passkey) is *This device could not make a passkey
here.* (`formError.ts::passkeyFormErrorText`), never "Could not reach the server". A 429 is `Too many attempts — try again in
Ns`. A session the server ended is *Your session has ended — log in again.*; one that could not
reach the server is *Could not reach the server — you are still logged in.* Buttons, tabs and titles are **sentence case**. An empty state ends with a
...
`Open X's profile`. A device the browser will not let the app notify is **blocked** — the word
in the bell, in Settings' status line and nowhere else in a second spelling (`Denied`, `Off`,
`Disabled` all mean other things here) — and the sentence that follows it says **where** to undo
it, because the app cannot: `The app cannot ask again — allow them in your browser or device
settings.`
...
- **A "see more" link is muted text + a chevron, never accent** (C14). One look, one class string
  ...
  **empty** About the trigger is gone for everyone (there is nothing pinned to comment on, and the
  ...
  owner, 16px for a visitor — because a way in that hid there would leave an owner
...
| Telling one person something happened | `NotificationBell` (`ui/shell/`) + `notificationText.ts` | seven kinds, one icon each, all `h-4 w-4` lucide: `Reply` `comment_reply`, `BookOpen` `guestbook`, `Hand` `poke`, `Lightbulb` `idea_created`, `MessageSquare` `idea_comment`, `ThumbsUp` `idea_vote`, `ListChecks` `idea_status` (P3). Every headline and detail line for all seven lives in `notificationText.ts` — never inline in the component — and a row reads *headline* (who did what) over *detail* (`snippet`, or the idea's title, or `title · snippet`) over `timeAgo`. **The bell is also where the app says this device is blocked from push** (Q-E): `pushSetupState === "blocked"` swaps `Bell` for `BellOff` in `text-warn` **inside the same 36px button** — a glyph swap, never a second element, because the top bar's right box is one control wide and its width is what keeps the title centred (§10.5). It is not the unread count and cannot be mistaken for it: the count is an accent pill in the opposite corner of the same button and the two are routinely both there. `warn`, not `error` (§2): nothing failed — the reader denied the permission. Opening the bell shows one `bg-warn/10` strip above the list carrying `PUSH_BLOCKED_TITLE` over `PUSH_BLOCKED_HINT` (`push/pushSetup.ts`, the one place either sentence is written, shared with the Settings panel), and **no button at all**: a denied permission can never be re-prompted, so anything offering to turn push on here would silently do nothing. Not dismissible — a mark on a control that is already there is not a banner |
...
| A list of one-line rows | `List` / `ListRow` | leading · title · subtitle · trailing, hairline separators, and the row's action as a **stretched overlay** (`absolute inset-0 z-0`) so `trailing` can hold its own buttons without nesting one control in another. Reach for it whenever the row fits that shape (players admin, tournaments, nav-ish lists). **`overlay`** (L11) replaces the built-in `Link`/`button` overlay with a caller's own — the Players admin row passes `<PlayerLink stretched …/>` there, so the row's door is the one that asks who may open a profile — and the class string both use is exported once, `LIST_ROW_OVERLAY_CLASS` |
...
| Identity → profile | `PlayerLink` | the only way an avatar/name becomes a link; hugs its text, stops click/Enter from bubbling so a row keeps its own action, `decorative` for an avatar that duplicates the name link. Never nest it in another `<a>`. **An author's byline is an identity** (Q-F): the guestbook, the tournament/match comments feed, the Ideas board and a cup reign's "took it from"/"ended by" all go through it, and an unattributed author ("General") stays a plain `<div>` because it has no profile to open. A `decorative` wrapper around an `AvatarCircle` carries **`inline-flex shrink-0 rounded-full`** — the `<a>` would otherwise open a line box and hand the row the font's descender space (both feeds are measured to the pixel), and a `rounded-xl` focus ring around a disc is the wrong shape. **It is also how a row keeps two meanings**: the guestbook row's own `onClick` marks an unread message read, and because `PlayerLink` stops the click, tapping the name navigates and leaves it unread while the rest of the row still marks it read. Where a tap already means something more specific — open this match, open this matchup, select this player in a picker — the identity is **not** linked. **Who may open a profile is answered inside `PlayerLink` and nowhere else** (L11), through `hooks/useProfileAccess.ts`, from the roster the client already holds: a player in it is a door; one outside it is **plain text** — a `<span>` with the link's classes minus the hover — titled *Not in your group*; and a player from another group wears the **`Users` mark**, 12px, `text-text-muted`, after the name, never on the `decorative` avatar beside it. The site admin may open anyone, so for him a foreign player is a door **with** the mark. The server refuses independently, into the profile page's own `EmptyState` "This profile is in another group." inside `PageLayout`. **`stretched`** renders the link as a row's whole-area overlay (`LIST_ROW_OVERLAY_CLASS`, an `aria-label`, no children) — the standings row and the Players admin row use it, which is how the app's last `div role="button"` went (§11) |
...
| A screen before the app | `AuthScreen` (`pages/auth/`) | the bare layout every auth page shares — `/login`, `/register`, `/reset`, `/recover`, `/verify-email` (E3), and "not in a group yet" — rendered **outside** `AppShell`: no top bar, no tab bar, no sidebar, no toast viewport. `min-h-screen bg-bg-default` with the body's safe-area padding, one `max-w-sm mx-auto px-4` column, the 96px `rounded-2xl` logo (`/icon-512.png`) and "Lorbeerkranz" in `text-xl font-semibold` above **one** `card`, and a `below` slot under the card for the muted "see more" lines (§6: "New here? Register with a code ›", "Already have an account? Log in ›", "Know your password? Log in ›", and on the login screen, before "New here?", "Lost your passkey or password? ›"). Measured at 390 / 1280 in both themes: card 358 / 384px wide, centred to the pixel, `a a` = 0 |
| A form's own error | one line under the form, never a toast | `text-xs text-error` with `role="alert"` — the server's own sentence verbatim (`pages/auth/formError.ts::formErrorText`), else "Could not reach the server — …". A toast cannot carry it: on the auth screens there is no toast viewport, and on a form it would float away from the field it is about. "Your session has ended — log in again." is the same shape in `text-warn`, above the login form, and only when a 401 ended a session that was open |
| Too many attempts | `RetryCountdown` (`pages/auth/`) | the **one** 429 line, on login, register, reset, redeem, the password editor, the passkey button, recover, verify-email and Settings → Email (one countdown holds both halves of `PasskeyOrPassword`): `text-xs text-warn`, "Too many attempts — try again in Ns", ticking from the server's `retry_after`, holding its own action disabled and nothing else (the passkey button's countdown never holds up "Log in"); `null` at 0 |
| A password field | `PasswordField` (`pages/auth/`) | the **one** password input, used by login, register, reset and Settings: an `Eye`/`EyeOff` `size="sm" iconOnly` toggle as its trailing control, **named after its field** ("Show current password", "Show new password"), and — for a new password — the live hint `At least 15 characters` (`MIN_PASSWORD_LENGTH`, never a literal), muted until met and `text-text-normal` once it is (`data-password-hint`). **No confirm field anywhere**: the eye toggle is the confirmation (§9b's light input). No composition rule is ever shown, because none exists |
| An invite code | `InviteCodeField` + `inviteCode.ts` (`pages/auth/`) | the **one** code input (register, "not in a group yet", Settings → Groups): it shows `ABCD-EFGH`, uppercases, drops everything outside the server's alphabet (so `O`/`0`/`I`/`1` never appear), caps at 8, and hands its caller the **raw** code; a `?code=` in a register link is read by the same `normalizeInviteCode`. `autoCapitalize="characters"`, `autoComplete="one-time-code"` |
| Setting a new login from a link | `ResetPage` (`pages/auth/`), `/reset` | the **one** token page — the admin's link, the CLI's and the emailed recovery link all open it. Reads the token from the fragment (or `?token=`) **once** through `useLinkToken.ts` and strips it from the address bar before the first paint and before the only request (the router's `replaceState`); a reload therefore shows the `EmptyState` *"This link is not complete — ask for a new one from the login screen or the admin."*, by design — the link works once and is kept nowhere. Inside the card: **Set a new login** (`text-lg font-semibold`), the muted *"This link works once. Using it signs out every other device."*, then `PasskeyOrPassword` — **Create a passkey** first, the password form (`PasswordField` + a full-width solid **Set password**, `KeyRound`) behind **Use a password instead**. A closed Face ID sheet spends nothing and the link stays live. Card 194px at 390, 272 with the password form (E3) |
| Choosing a credential | `PasskeyOrPassword` (`pages/auth/`) | the **one** "passkey or password" block (E3), used by the register and reset pages — the passkey first wherever a credential is chosen (Roli). A solid full-width **Create a passkey** (`KeyRound`; "Waiting for your passkey…" while the sheet is up), then a ghost full-width **Use a password instead** (`ChevronDown`); pressing the ghost swaps in the caller's own `passwordForm` with a muted **Create a passkey instead ›** above it — the way back, discarding nothing but the choice. **Where `passkeysSupported()` is false it renders the password form alone — no toggle, no word "passkey"** (the phone on plain-http dev; an old browser). The prop shape is the design: a page hands it its existing form. Enter in a field while the passkey half shows submits nothing. The error line is `passkeyFormErrorText` — the server's sentence, else *This device could not make a passkey here.*, else the network line |
| Asking for a way back in | `RecoverPage` (`pages/auth/`), `/recover` | `AuthScreen`; **Get back in**, the muted *"Enter the email address on your account."*, an `Input type="email"` labelled **Email** (`inputMode="email"`, `autoComplete="email"`, `autoCapitalize="none"`), a solid full-width **Send me a link** (`Mail`), `RetryCountdown`. After a submit — success **or** any error that is not a 429, because the server's answer is the same for every address and so is ours — the form is replaced by the one sentence *"If that address is verified, a link is on its way. Check your spam folder too."* (`role="status"`, muted) and the intro line goes. `below`: "Know your password? Log in ›" and, always, the muted `text-xs` *"No verified email on your account? Ask the admin for a reset link."* Card 196px at 390 |
| Confirming an email | `VerifyEmailPage` (`pages/auth/`), `/verify-email` | `AuthScreen`; reads and strips the token like the reset page (`useLinkToken`); **a button, not an automatic POST** — a mail scanner that runs JavaScript would otherwise spend the link before the person taps it (measured: no request in 1.5 s on load, one on the tap). **Confirm your email** and a solid full-width **Confirm** (`MailCheck`); success → *"Verified. You can close this and go back to the app."* plus, only when logged in, **Back to Settings ›**; failure → the server's sentence; no token → *"This link is not complete — ask for a new one in Settings → Account."*; a `below` "Log in ›" only while anonymous. It opens in Safari from Mail, so it works with no session (the endpoint is public). Card 102px at 390 |
| A passkey on the login screen | a ghost `Button` "Use a passkey" (`Fingerprint`) | full width, **after** the password form and a `divider` — a person with a password is not made to hunt for it — and rendered only where `browserSupportsWebAuthn()`, which hides it on the phone against dev (plain http is not a secure context). Under it, one muted `text-xs` line — *"Passkey on your phone, logging in on a computer? Choose it in the passkey prompt and scan the code with your phone."* — the cross-device (QR) case said once where it applies, only where the button shows (E3; login card 323px at 390, 307 at 1280). The login is the one screen that keeps the **password first**: it chooses no credential, it uses one. **A cancelled system sheet is not an error**: a closed Face ID / passkey sheet (`NotAllowedError`) says nothing at all, anywhere in the app; a refusal is the server's one sentence on the form's error line |
| My account | `SecuritySection` (`pages/settings/`) in `SettingsSection` cards, Settings → Account | five groups, in this order. **Devices**: a `ListRow` per session, `Since <date> · last seen <date, time>`, the current one wearing a `pill-default` **this device** and dropping "last seen" (it is in use now; at 390px the pill truncated the line otherwise) and carrying **no** sign-out; every other row a ghost `LogOut` icon button → `ConfirmDialog` **without** a red block (reversible: log in again); a footer ghost "Sign out other devices" only while another device exists. **Passkeys**: a row per passkey (label, `Added <date> · last used <date, time>` or `· not used yet`, a `pill-default` "synced" when backed up) with a ghost `Trash2` → `ConfirmDialog` **with** the red block, subtitle "Every device will be signed out, including this one." — after it the app logs out and lands on `/login` without the "session has ended" line (nobody was surprised); **the last way in** (no password, one passkey) is a disabled button whose title *and* a line under the list carry the server's own 409 sentence, because a tooltip does not exist on a phone; then an optional **Name** field and a full-width solid **Add a passkey** (`KeyRound`), replaced by "This browser cannot make passkeys." where WebAuthn is missing; and, only where passkeys are supported, one muted `text-xs` hint under the list — *"Logging in on another device? Choose your passkey there and scan the code with this phone — or add a password below."* (E4). **Password**: read view or editor, never both (§9b) — a full-width ghost toggle **Change password** / **Set a password** with a chevron and `aria-expanded`; the editor is the current field (only when there is a password), the new `PasswordField`, one full-width solid **Save**, the error line and `RetryCountdown`; the toggle is the way out and discards the draft. "This is the password you were given — change it (15 characters or more), or add a passkey." in `text-warn` above it while the password is the migrated one and no passkey exists (the number read from `MIN_PASSWORD_LENGTH`). **Remove password** is a muted full-width ghost button, only with a password *and* a passkey, `ConfirmDialog` **without** a red block (reversible by "Set a password" right there). **Email** (E4, `EmailSection`), four states off `useAuth()`, never re-derived: *mail off* → one muted line *"Email is not set up on this server. If you get locked out, ask the admin for a reset link."* and nothing else; *none* → the muted *"A verified email lets you get back in if you lose your passkey or forget your password."*, an `Input type="email"` **Email**, a full-width solid **Send verification link** (`Mail`), the error line and `RetryCountdown`; *pending* → a `ListRow` with the address and a `pill-default` **pending**, the muted *"Open the link we sent to confirm it — on any device. Check your spam folder too."*, ghost **Send again** (`RefreshCw`) and ghost **Change**, and the section re-asks `/me` when the reader comes back (the link opens in Safari); *verified* → the row with **verified**, ghost **Change**, and a muted full-width ghost **Remove email** → `ConfirmDialog` **with** the red block ("Remove your email address?" — *"You will not be able to get back in by email until you verify a new one."*). A pending change lists the verified address above it, still working. The success line *"Verification link sent to <addr>."* shows only while that address is still pending; *"Email removed."* after a removal. Card 204 / 222 / 202px (none / pending / verified) and 90 when mail is off, at 390. **Groups**: a `ListRow` per membership with the role as a `pill-default` `Pill`, then `InviteCodeField` + a ghost **Join**. Measured at 390 / 1280 in both themes: the cards at x=16 / w=358 and x=424 / w=672 |
| "Secure your account" | `SecureAccountNotice` (`ui/shell/`) | the `PushSetupNotice` shape — first in `<main>`, above the route, `card mb-3 border-warn/40 bg-warn/10` (a `.card`, never an `.inset`, so light keeps the tone, §3), a `KeyRound` in `text-warn`, the sentence, and one solid small `Link` to `/settings?tab=account` carrying `NAV_JUMP_STATE`. `warn` because nothing failed (§2). **Not dismissible** (Roli): it stops by itself. **Its condition — decided by Roli on 2026-09-24, and written here and in the component's JSDoc, nowhere else:** an account is secured when it has **both** a verified email **and** a secure login (a passkey, or a password set on this code — `loginSecure`, the server's word), and **the email step exists only while the server can send** (`emailAvailable`), so the strip can always be cleared: `needsEmail = emailAvailable && !emailVerified`, `needsLogin = !loginSecure`, `shown = authed && (needsEmail \|\| needsLogin)`, never on Settings → Account and never on the auth screens. It says which step is missing — both, WebAuthn supported: *"Secure your account — add an email address and a passkey."* / **Secure account**; both, unsupported: *"… add an email address and set a new password."* / **Secure account**; the email: *"… add an email address."* / **Add email**; the login, supported: *"… add a passkey."* / **Add a passkey**; the login, unsupported: *"… set a new password."* / **Set a password**. `passkeysSupported()` chooses the words, **never the visibility** (L9's "hidden where WebAuthn is missing" is gone). While shown it re-asks `/me` on return (`useRefreshMeOnReturn`, one call per 2 s), because the verify link opens in Safari. It lives in the page column, so it cannot move the top bar's title (Q13, 195.0 with and without). Measured (E4): **358×66** at 390 for a one-line sentence (the tab strip +78px) and **358×86** for a two-line one (+98px); **992×58** at 1280 for every sentence (+70px), in both themes |
| The admin page | `AdminPage` (`pages/admin/`), `/admin`, owner+ | `PageLayout title="Admin"` with `SectionTabs` **Accounts** (`Users`, default) · **Invites** (`Ticket`), the body in `mx-auto w-full max-w-2xl`. *Accounts*: `section-head` "Accounts", for the site admin one line from `/admin/mail-status` above the list (muted *"Email: <description>"* when configured, `text-warn` *"Email is not set up on this server — recovery by email is off."* when not — an owner never asks), a `ChipGroup` **All** / **Logged in** / **Not secured** (E4 — the strip's two steps read off the server's `login_secure` / `email_state`, `notSecured(a, emailAvailable)` written once; empty state *"Every account is secured."*), then one `List` of `ListRow`s — `AvatarCircle h-10 w-10` (**no cups**: this is not a present-tense surface), the name with a `pill-default` **site admin** or **owner**, and a subtitle that **leads with the login state** (`not secured` in `text-warn` — the same word as the filter chip above it, one word for one state per §5b — or `no login`), then — only while the server can send — `no email` / `email pending` (muted; the address itself is never shown), before `N devices · last seen <date, time>`, because at 390px the tail is what truncates and the state is what Roli asked to see. Trailing `size="sm" iconOnly` ghosts: `UserCog` (make/remove owner, owner+, only on rows whose role is `owner` or `editor`) then `KeyRound` (a reset link, **site admin only**). The row's own tap opens `SessionsSheet` for the site admin only; for an owner the row is not a control at all (the server would 403 the sheet — no overlay, no chevron). *Invites*: `section-head` "Invite codes", a note `Input`, a **full-width** solid **Create code**, the `ShownOnce` box, then "Live codes" as a `List` (note or "No note", `created <ts> · by <name> · expires in N min`, a `Trash2` revoke — **with** the red block, because it deletes a stored row). The title "Admin" centres at 160.0 / 195.0 / 215.0 on the phone (Q13) |
| A player's devices, for the admin | `SessionsSheet` (`pages/admin/`) | a `Modal` titled with the player's name, a `List` of their sessions (`device_label`; `created · last seen · <kind>`), the admin's own current one marked **this device** with no button, every other a ghost `LogOut` → `ConfirmDialog` "Sign this device out?" (no red block: "They can log in again."), and a footer ghost **Sign out every device** that names the count and says "— this one included" when it is |
| A secret shown once | `ShownOnce` (`pages/admin/`) | the **one** box for an invite code or a reset link: `inset`, the secret in `font-mono` (a code `text-2xl tabular-nums tracking-wider` as `ABCD-EFGH`; a link `text-xs break-all`), **Copy** buttons over `utils/clipboard.ts::copyText` (the text is selected when the clipboard is blocked), and "It will not be shown again." always last. **A secret is shown once and a list never repeats it** — the server returns an invite code only in the response that created it, and the live-codes list is built without it |
...
- **A flat comment list under a row that is itself an `inset` takes the reply shape** — flat rows
  ...
  **names the count** (`fmtCount`); at zero comments the toggle reads **Comment** for someone who
  can post one and is **not rendered at all** for someone who cannot, because a control that does nothing is
...
- **An item never hosts its own thread — it arms the one feed that does** (K1–K3). A profile's
  ...
  wants 0 — with `lg:bottom-0` beside it, those two are what Q2, Q14 and Q-D stand on). Someone who
...
- **A password is edited in place, like everything else** (L7). Settings → Account → Password is a
  read view or an editor, never both: one full-width toggle that names the job ("Change password",
  or "Set a password" for an account without one) opens the fields under it, one primary **Save**
  fills the row, and the toggle is the way out — a second Cancel would be the same job twice. After
  a save the read view says what happened ("Password changed. Your other devices stay signed in.").
  The worked example is §7's "My account" row. **The Email group follows the same rule** (E4): its
  **Change** is a toggle with a chevron and `aria-expanded` that opens the field under the row and
  is the way out again (the draft is discarded), and a success line that no longer describes the
  row — a "link sent" line under a row that now says **verified** — is removed, not left to go
  stale.
...
**Every location is basename-relative, and the hierarchy never sees `/g/`** (L10). The address bar
reads `/g/altherren/live/3`, but react-router's `basename` hands every hook, link and rule below
`/live/3`: `routeHierarchy`, `navConfig`, `navStack`, `lastLocation` and `useTabParam` did not change
and must never learn the prefix. A path the backend emits arrives absolute and goes through
`basename.ts::toRouterPath` before it becomes a navigation; nothing else in `src/` spells `/g/`.

**Every location is one of two things.**

- A **destination** — anything the bottom bar, the sidebar or the drawer points at (Dashboard,
  Tournaments, Friendlies, Stats, Players, Clubs, Ideas, Admin, Settings), plus the auth screens
  (`/login`, `/register`, `/reset`, `/recover`, `/verify-email`, which render outside the shell and
  have no back affordance)
  and any URL that matched nothing. Destinations are siblings laid out in a bar, not rungs of a
  ladder. **`/admin` is a destination for owners and the site admin only** (L6): in the sidebar and
  the drawer, after Ideas, and — like Clubs and Ideas — never in the bottom bar, which stays at
  five.
...
5. **Back and the menu coexist, inside a frame that never moves** (Q13). The phone's top bar is one
   ...
   variable width moves the title. Anything the bell has to say about *itself* therefore happens
   **inside** its own 36px box (Q-E's `BellOff`, §7), never beside it; measured on all nine
   top-bar routes at 320 / 390 / 430px, the title centre is identical with the mark and without. Back took the screen edge until 2026-09-16, and pushed the menu
...
- Do use `ListRow` where a row is leading · title · subtitle · trailing, and `list-divided` +
  ...
  make a `<div>` a `role="button"` — **the last one, the standings row, went in L11**
  (`[role="button"]` on a live tournament 4 → 0; three code comments remain, each saying not to).
- Do let `PlayerLink` decide whether a name is a door; don't ask "may I open this profile" anywhere
  else in the browser, and don't make a name a link that the server would refuse (§7, L11).
- Do show a secret once, in `font-mono`, with a Copy button, through `ShownOnce`; don't let a list
  repeat it (§7, L6).
- Do put a form's own error on one line under the form; don't send it to a toast (§7, L4).
- Do stay silent when the person closed a system sheet (Face ID, the passkey picker); don't print
  an error for a choice they made (§7, L9).
