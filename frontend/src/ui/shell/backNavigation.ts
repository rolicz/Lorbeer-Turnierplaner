...
import { canPop as historyCanPop, previousEntryPath } from "./navStack";
 ...
 * The rule, in full:
 *
 * - On a page you went **into** (`placeOf().inside`), back means **one level
 *   up**, identically whether you walked in, followed a deep link or opened a
 *   push notification. It pops when the entry behind us already *is* that parent
 *   — free, and better, because the parent comes back with its scroll offset
 *   (N2), its open tab and its data — and otherwise navigates to it.
 ...
 *   the browser, iOS and Android all do between siblings. With nothing behind it
 *   — a cold deep link — it goes home rather than out of the app.
...
export function resolveBackAction({ pathname, search = "", state, canPop, previousPath }: BackInput): BackAction {
