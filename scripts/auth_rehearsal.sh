...
# deleted from the copy before anything boots, and no VAPID key is ever set.
#
...
#   3  first boot of THIS tree on the copy: the migration's log lines, every account logs in,
#      the same answers as the baseline, the JWT exchange, timing; a passkey is registered in
#      Chromium and old deep links are opened (unless --no-browser)
...
#      the old code's JWT exchanges, the passkey from step 3 signs in from a fresh browser
#   6  a third boot (must migrate nothing) and the five escape-hatch commands
#   7  one boot with docker-compose's production environment (the boot guard, /health, Secure)
#   8  hash_password timing; the snapshot is unchanged
#
# Ports (L13's row): backend 8244, old backend 8245, vite 8264. KEEP_WORK=1 keeps the work dir.
set -uo pipefail
...
boot_lines() { grep -E "Cup defs|Auth migra|Cups imported|the file is only a seed|seeded|swept|backfilled|reconciled|DB initialized|Traceback|Error" "$1" | sed -E 's/^.*(INFO|WARNING|ERROR)[^A-Za-z]*//' | sed 's/^/  log: /'; }
...
t0=$(date +%s%N)
if start_backend "$REPO/backend" $B_NEW "$DB" "$WORK/new1.log" APP_ENV=development AUTH_DEV_ORIGIN=1; then
  ...
  if [ $BROWSER -eq 1 ]; then
    ( cd "$REPO/frontend" && . "$REPO/scripts/node-env.sh" && exec env BACKEND_ORIGIN="http://127.0.0.1:$B_NEW" \
      VITE_API_BASE_URL=/api VITE_WS_BASE_URL= node node_modules/vite/bin/vite.js --port $V --strictPort --host 127.0.0.1 ) >"$WORK/vite.log" 2>&1 &
    VITE_PID=$!
    PIDS+=("$VITE_PID")
...
if start_backend "$WORK/old/backend" $B_OLD "$DB" "$WORK/old.log"; then
...
if start_backend "$REPO/backend" $B_NEW "$DB" "$WORK/new2.log" APP_ENV=development AUTH_DEV_ORIGIN=1; then
...
if start_backend "$REPO/backend" $B_NEW "$DB" "$WORK/new3.log" APP_ENV=development AUTH_DEV_ORIGIN=1; then
  ...
  checks escape --base "http://127.0.0.1:$B_NEW" --db "$DB" --py "$PY" --backend "$REPO/backend" --secrets "$WORK/secrets.json"
  stop "$SRV_PID"
